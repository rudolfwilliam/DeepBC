...
def train_vae(vae_img, train_set, val_set, config):
    trainer = Trainer(accelerator="auto", devices="auto", strategy="auto", gradient_clip_val=config["gradient_clip_val"], callbacks=[generate_checkpoint_callback(config["name"], config["ckpt_path"]), 
                                 generate_early_stopping_callback(patience=config["patience"])], default_root_dir="./project_files/celeba/scm", max_epochs=config["max_epochs"])
...
def main():
    ...
    data = CelebaContinuous(cont_attr_path="./project_files/celeba/data/predictions/preds.pt")
    config = load(open("./project_files/celeba/scm/config/" + args.name + ".json", "r"))
    # overwrite checkpoint path
    config["ckpt_path"]  = "./project_files/celeba/baselines/non_causal_CE/trained_models"
