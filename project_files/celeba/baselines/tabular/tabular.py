...
from deepbc.scm import StructuralEquation
from deepbc.optim import backtrack_gradient, backtrack_linearize
from deepbc.utils import override
from project_files.celeba.baselines.tabular.train_regressor import Regressor
...
class LinearSE(StructuralEquation):
    def __init__(self, name):
        ...
        self.regressor = Regressor(ckpt_path="./project_files/celeba/baselines/sparsity_on_observed/trained_models/checkpoints/", name=name)
...
def tab_CE(scm, vars_, vals_ast, ckpt_path="./project_files/celeba/baselines/tabular/trained_models/checkpoints/", linearization=False, sparse=True, n_largest=1, lr=1e-2, num_it=500, lambda_=1e3, verbose=False, **us):
