...
def tab_CE(scm, vars_, vals_ast, ckpt_path="./celeba/baselines/tabular/trained_models/checkpoints/", linearization=False, sparse=True, **us):
    ...
    if linearization:
        xs_ast = backtrack_linearize(scm=scm_attr, vals_ast=vals_ast, vars_=vars_, sparse=sparse, num_it=100, lambda_=1e2, **xs_copy)
    else:
        xs_ast = backtrack_gradient(scm=scm_attr, vals_ast=vals_ast, vars_=vars_, num_it=500, lr=1e-1, lambda_=1e5, **xs_copy)
